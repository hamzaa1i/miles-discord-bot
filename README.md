...
- Mention-based chat: `@ao <message>` (with natural-language intent parsing — try "ban @user spam" or "weather in Lahore")
- Slash chat: `/chat`, `/ask`
...
> Note: Gambling/casino-style features are intentionally excluded (blackjack / coinflip / slots in cogs/games.py are not part of the economy shop and don't pay out real coins beyond their own bet pool).
...
- Moderation grouped under `/mod`:
  - kick / ban / unban / timeout / untimeout / warn / warn list / warn clear / purge / logs
  - nuke, role add/remove, nickname, softban, slowmode, lock, unlock
- AutoMod: anti-spam, word filter, logging channel

### 🔧 Utility
- `/weather`, `/math`, `/password`, `/encode`, `/decode`, `/timestamp`
- `/snipe`, `/editsnipe`, `/afk`, `/firstmessage`, `/botinfo`, `/uptime`, `/ping`
- `/remind`, `/note`, `/notes`
...
### 📋 Other
- Tickets, suggestions, giveaways, productivity, marriage, reputation, birthdays, counting, starboard, polls
...
- `/` → basic HTML "ao is online" response with current timestamp
...
│   ├── image_generator.py   # Shared safe font loader (FIX 5)
│   └── professional_embeds.py
└── cogs/
    ├── ai_chat.py           # @ao chat + intent execution
    ├── ai_features.py       # /summarize /translate /explain /code /debug /story /poem /advice /define /tldr /roast_server /aipoll
    ├── fun.py               # /joke /meme /fact /quote /pickup /wouldyourather /8ball /roast /compliment /hack /howsmart /roastme /ship /rate /roll /flip /mock /clap /reverse /say /ascii /emojify /truth /dare /tod /choose /topic /would
    ...
    ├── moderation.py        # /mod kick/ban/unban/timeout/warn/purge/nuke/role/nickname/softban/slowmode/lock/unlock/logs
    ├── utility.py           # /weather /math /password /encode /decode /timestamp /snipe /editsnipe
    ...
    ├── help.py              # /help with category buttons
    ...
    ├── automod.py / server_stats.py / trivia.py / autorole.py / counting.py
    └── __init__.py
```
